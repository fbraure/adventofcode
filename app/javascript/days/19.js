...
const finalize = (initRules, ruleName, finalRules) => {
  let valids = [];
  let valids1;
  let valids2;
  let valids3;
  let requestedRules = initRules.get(ruleName)
  if(['"a"','"b"'].includes(requestedRules)){
    finalRules.set(ruleName, [requestedRules.replaceAll('"','')])
  }else{
    // requestedRules '89: 96 33 | 121 96',
    requestedRules.split("| ").forEach(requestedRule=>{
      // requestedRule 96 33
      valids1 = finalRules.get(requestedRule.split(" ")[0]);
      valids2 = finalRules.get(requestedRule.split(" ")[1]);
      valids3 = finalRules.get(requestedRule.split(" ")[2])
      valids1.forEach(valid1=>{
        if(undefined !== valids2){
          valids2.forEach(valid2=>{
            if(undefined !== valids3){
              valids3.forEach(valid3=>{
                valids.push(valid1+valid2+valid3);
              })
            }else{
              valids.push(valid1+valid2);
            }
          })
        } else {
          valids.push(valid1);
    ...
    finalRules.set(ruleName, valids)
  }
  initRules.delete(ruleName)
}

const pack = (initRules) => {
  let finalRules = new Map()
  let needRules;
  while(initRules.size > 0){
    initRules.forEach((rules, ruleName, map) =>{
      // console.log(rules)
      if(['"a"','"b"'].includes(rules)){
        finalize(initRules, ruleName, finalRules)
      }else{
        needRules = rules.replaceAll("| ","").split(" ")
        if(needRules.every(rule=>finalRules.has(rule))){
          finalize(initRules, ruleName, finalRules)
        }
      }
    })
    // console.log(`init:${initRules.size} final:${finalRules.size}`)
  }
  // console.log(finalRules)
  return finalRules
}
const isValid = (test, rules) => {
  // console.log(rules)
  let found = false
  rules.forEach(rule=>{
    if(rule === test){
...
export const exec_19 = () => {
  let result1 = 0;
  let initRules = new Map()
  inputs.forEach((line, index) => {
    initRules.set(line.split(": ")[0], line.split(": ")[1])
  });
  initRules = pack(initRules);
  tests.forEach(test=>{if(isValid(test, initRules.get("0"))){result1++}})
  console.log(result1);

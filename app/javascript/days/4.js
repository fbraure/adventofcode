...
const ref1 = ["byr","iyr","eyr","hgt", "hcl", "ecl", "pid"]
const ref2 = ref1 + "cid"

const valid1 = (passport)=> {
  passport = passport.map((pass_field)=>pass_field.split(":")[0])
  if( passport.filter(word => ref2.includes(word)).length === ref2.length || passport.filter(word => ref1.includes(word)).length === ref1.length){
    return 1
  }
  return 0
}
const valid2 = (passport)=> {
  let nbvalid = 0
  passport.map((pass_field)=>pass_field.split(":")).forEach(element => {
    ...
    switch(element[0]){
      ...
      case 'hgt':{
        if(element[1].substr(-2,2) === "cm" && parseInt(element[1]) >= 150  && parseInt(element[1]) <= 193) {
          nbvalid++
          // console.log(`hgt ok`)
        }
        else if(element[1].substr(-2,2) === "in" && parseInt(element[1]) >= 59  && parseInt(element[1]) <= 76) {
          nbvalid++
          // console.log(`hgt ok`)
        }
        break;
...
export const exec_4 = () => {
  ...
  let result2 = null;
  let nbPassport = 0;
  let nbPassportValid1 = 0;
  let nbPassportValid2 = 0;
  let iLine = 0;
  let passport = ""
  while(iLine <= inputs.length) {
    if(inputs[iLine] === ""){
      ++nbPassport
      passport = passport.split(" ");
      passport.shift();
      nbPassportValid1 += valid1(passport)
      nbPassportValid2 += valid2(passport)
      passport = ""
    }
    else{
      passport += " " + inputs[iLine]
    }
    ++iLine;
  }
  result1 = `4-1 : Nb of Passport: ${nbPassport} Nb of Passport valid: ${nbPassportValid1}`
  result2 = `4-2 : Nb of Passport: ${nbPassport} Nb of Passport valid: ${nbPassportValid2}`

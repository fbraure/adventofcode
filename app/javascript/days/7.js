...
const isInBag = (upperBagsMap, upperbag) => {
  // console.log(upperBagsMap)
  // console.log(upperbag)
  let lowerBags = upperBagsMap.get(upperbag)
  // console.log(lowerBags)
  let found = false
  // lowerBags.includes("shinygold")
  lowerBags.forEach(lowerBag=>found |= lowerBag[0] == "shinygold")
  if(!found){
    lowerBags.forEach(lowerBag=>found |= isInBag(upperBagsMap, lowerBag[0]))
  }
  return found ? 1 : 0
}
const countBags = (upperBagsMap, upperbag) => {
  let count = 1;
  let lowerBags = upperBagsMap.get(upperbag)
  lowerBags.forEach(lowerBag=>{
    count += lowerBag[1] * countBags(upperBagsMap, lowerBag[0])
  })
  return count
}
export const exec_7 = () => {
  let result1 = 0
  let result2 = null
  const upperBagsMap = new Map()
  inputs.forEach(input =>{
    const words = input.split(" ")
    const lowerBags = []
    for(let i = 4; i < words.length; i+= 4){
      if(words[i] !== "no"){
        lowerBags.push([words[i+1]+words[i+2].replace(".",""), parseInt(words[i])])
      }
    }
    upperBagsMap.set(words[0]+words[1], lowerBags)
  })
  upperBagsMap.forEach((lowerBags, upperBag, upperBagsMap)=>{
    result1 += isInBag(upperBagsMap, upperBag)
  })
  result2 = countBags(upperBagsMap, "shinygold") - 1
  result1 = `7-1 : Bags containing Shiny: ${result1}`
  result2 = `7-2 : Bags in Shiny: ${result2}`

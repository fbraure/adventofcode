...
export const exec_6 = () => {
  ...
  while(iLine <= inputs.length) {
    if(inputs[iLine] === ""){
      const toto = new Set()
      passport.split("").forEach((letter) => {
        toto.add(letter);
      });
      nbCount += toto.size -1 // -1 pour enlever " "
      passport = passport.split(" ")
      ref = passport[1].split("") // -1 pour enlever " "
      for(let i = 1; i < passport.length ; ++i){ // -1 pour enlever " "
        ref = ref.filter(refLetter => passport[i].includes(refLetter))
      }
      nbCount2 += ref.length
      passport = ""
    }
  ...
  let result1 = `6-1 : SumOf Counts: ${nbCount}`
  let result2 = `6-2 : SumOf Counts: ${nbCount2}`

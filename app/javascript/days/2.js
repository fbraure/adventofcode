...
export const exec_2 = () => {
  let result1 = null;
  let result2 = null;
  let nbGreatPasswd1 = 0;
  let nbGreatPasswd2 = 0;
  inputs.forEach((input)=>{
    const subInput = input.split(":")
    const policy = subInput[0].trim();
    const letter = policy.split(" ")[1];
    const availables = policy.split(" ")[0]
    const password = subInput[1].trim();
    const regex = new RegExp(letter.trim(), 'gi')
    var count = (password.match(regex) || []).length;
    if (count >= parseInt(availables.split("-")[0]) && count <= parseInt(availables.split("-")[1])){
      ++nbGreatPasswd1;
    }
    if ((password[parseInt(availables.split("-")[0]) -1] === letter && password[parseInt(availables.split("-")[1]) -1] !== letter)
    || (password[parseInt(availables.split("-")[0]) -1] !== letter && password[parseInt(availables.split("-")[1]) -1] === letter)){
      ++nbGreatPasswd2;
      // console.log(input)
    }
  })
  result1 = `2-1 : Nb Of Available Passwords: ${nbGreatPasswd1}`
  result2 = `2-1 : Nb Of Available Passwords: ${nbGreatPasswd2}`
